    ...
    if(isset($_POST['add-book-form']) && !empty($_POST) ){
        ...
        if($_FILES['book-file']['size'] > 0){
            ...
            if(in_array(strtolower($file_ext[1]),$extensions)=== false){
             ...
             if(empty($errors)==true){
                $bookName = $_FILES["book-file"]["name"] ;
                $result =   explode(".",$bookName);
                // var_dump(end($result));de
                $book = time(). "_".  rand(100, 999999) . '.' . end($result);
                $book_file  = $book ;
        ...
        if($_FILES['book-image']['size'] > 0){
            ...
            if(in_array(strtolower($file_ext[1]),$extensions)=== false){
             ...
             if(empty($errors)==true){
                $result = explode(".",$_FILES["book-image"]["name"]);
                $image = time(). "_". rand(100, 999999) . '.' . end($result);
                $bookImage = $image;
